...
class OpenAIProvider:
    ...
    async def _attempt(self, *, prompt: str, schema: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            raise LLMCallError("openai request timed out") from exc
        except httpx.HTTPError as exc:
            raise LLMCallError(f"openai http error: {exc}") from exc

        if response.status_code != 200:
            raise LLMCallError(f"openai error {response.status_code}: {response.text}")
        ...
        try:
            ...
            raise LLMCallError(f"openai returned non-json: {text!r}") from exc
