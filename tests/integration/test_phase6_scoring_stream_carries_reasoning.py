...
async def test_tier_change_event_enqueued_with_reasoning(
    ...
    enqueued: TierChangeEvent = enqueue.await_args.kwargs["event"]
    assert enqueued.reasoning is not None
    assert enqueued.reasoning.primary_drivers
