...
_FIXTURES = Path(__file__).parents[4] / "fixtures" / "patterns" / "isolation"
...
async def test_high_confidence_returns_match() -> None:
    text = "Don't tell your parents about this, they wouldn't understand our friendship"
    provider = FakeProvider(
        responses={
            _build_prompt_key(text, ()): {
                "confidence": 0.9,
                "evidence_excerpts": ["Don't tell your parents", "they wouldn't understand"],
            }
        }
    )
    from services.patterns.app.library.isolation import IsolationPattern
    ...
    match = matches[0]
    assert match.pattern_name == "isolation"
    assert match.signal_kind is SignalKind.ISOLATION
    ...
    assert "Don't tell your parents" in match.evidence_excerpts
...
async def test_below_threshold_returns_no_match() -> None:
    text = "this is just between us okay"
    provider = FakeProvider(
        responses={
            _build_prompt_key(text, ()): {
                "confidence": 0.49,
                "evidence_excerpts": ["just between us"],
            }
        }
    )
    from services.patterns.app.library.isolation import IsolationPattern
...
async def test_at_threshold_returns_match() -> None:
    text = "please keep this secret, I don't want anyone else knowing"
    provider = FakeProvider(
        responses={
            _build_prompt_key(text, ()): {
                "confidence": 0.50,
                "evidence_excerpts": ["keep this secret"],
            }
        }
    )
    from services.patterns.app.library.isolation import IsolationPattern
...
async def test_does_not_fire_on_non_minor_recipient() -> None:
    text = "Don't tell anyone about this, it's just between us"
    from services.patterns.app.library.isolation import IsolationPattern

    pattern = IsolationPattern(provider=FakeProvider(responses={}))
...
async def test_pattern_metadata() -> None:
    ...
    pattern = IsolationPattern(provider=FakeProvider(responses={}))
    assert pattern.name == "isolation"
    assert pattern.signal_kind is SignalKind.ISOLATION
    assert pattern.mode is DetectionMode.LLM


async def test_history_injected_into_prompt() -> None:
    text = "don't tell your mum we talked"
    history = ("hi", "hey how are you")
    provider = FakeProvider(
        responses={
            _build_prompt_key(text, history): {
                "confidence": 0.8,
                "evidence_excerpts": ["don't tell your mum"],
            }
        }
    )
    from services.patterns.app.library.isolation import IsolationPattern
