...
_FIXTURES = Path(__file__).parents[4] / "fixtures" / "patterns" / "platform_migration"
...
async def test_fires_on_migration_phrases(text: str) -> None:
    pattern = PlatformMigrationPattern()
    matches = await pattern.detect_sync(_ctx(text))
    assert len(matches) == 1
    assert matches[0].signal_kind is SignalKind.PLATFORM_MIGRATION_REQUEST
    assert matches[0].detection_mode is DetectionMode.RULE


async def test_no_fire_on_safe_text() -> None:
    pattern = PlatformMigrationPattern()
    matches = await pattern.detect_sync(_ctx("see you at school tomorrow"))
    assert matches == ()
...
async def test_positive_fixtures_fire(case: FixtureCase) -> None:
    ...
    matches = await pattern.detect_sync(_ctx(combined))
