...
_FIXTURES = Path(__file__).parents[4] / "fixtures" / "patterns" / "exclusivity_llm"
...
def _build_prompt_key(text: str, history: tuple[str, ...]) -> str:
    ...
    p = ExclusivityLLMPattern(provider=FakeProvider(responses={}))
    return p._render_prompt(text, history)


async def test_high_confidence_returns_match() -> None:
    text = "You're so mature for your age, no one gets you like I do"
    ...
    pattern = ExclusivityLLMPattern(provider=provider)
    matches = await pattern.detect_llm(_ctx(text))
    assert len(matches) == 1
    match = matches[0]
    assert match.pattern_name == "exclusivity_llm"
    assert match.signal_kind is SignalKind.EXCLUSIVITY
    ...
    assert "mature for your age" in match.evidence_excerpts


async def test_zero_confidence_returns_no_match() -> None:
    text = "great job on the game today"
    provider = FakeProvider(
        responses={
            _build_prompt_key(text, ()): {
                "confidence": 0.0,
...
async def test_below_threshold_returns_no_match() -> None:
    text = "you seem really smart for your age"
    provider = FakeProvider(
        responses={
            _build_prompt_key(text, ()): {
                "confidence": 0.54,
                "evidence_excerpts": ["smart for your age"],
            }
        }
    )
    from services.patterns.app.library.exclusivity_llm import ExclusivityLLMPattern
...
async def test_at_threshold_returns_match() -> None:
    text = "you understand things your friends never would"
    provider = FakeProvider(
        responses={
            _build_prompt_key(text, ()): {
                "confidence": 0.55,
                "evidence_excerpts": ["your friends never would"],
            }
        }
    )
    from services.patterns.app.library.exclusivity_llm import ExclusivityLLMPattern
...
async def test_does_not_fire_on_non_minor_recipient() -> None:
    text = "No one understands you the way I do"
    from services.patterns.app.library.exclusivity_llm import ExclusivityLLMPattern

    pattern = ExclusivityLLMPattern(provider=FakeProvider(responses={}))
...
async def test_pattern_metadata() -> None:
    ...
    pattern = ExclusivityLLMPattern(provider=FakeProvider(responses={}))
    assert pattern.name == "exclusivity_llm"
    assert pattern.signal_kind is SignalKind.EXCLUSIVITY
    assert pattern.mode is DetectionMode.LLM


async def test_history_injected_into_prompt() -> None:
    text = "you're not like other kids"
    history = ("hi", "hey")
    provider = FakeProvider(
        responses={
            _build_prompt_key(text, history): {
                "confidence": 0.8,
                "evidence_excerpts": ["not like other kids"],
            }
        }
    )
    from services.patterns.app.library.exclusivity_llm import ExclusivityLLMPattern
