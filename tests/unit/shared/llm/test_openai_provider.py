...
from shared.llm.openai_provider import _API_URL, OpenAIProvider
from shared.llm.provider import LLMCallError, LLMValidationError

pytestmark = [pytest.mark.unit, pytest.mark.asyncio]

_KEY = "sk-openai-test-key"  # pragma: allowlist secret
_SCHEMA = {
    "type": "object",
    "required": ["confidence"],
...
def _openai_response(payload: dict[str, object]) -> dict[str, object]:
    return {
        "id": "chatcmpl-01",
...
async def test_returns_valid_payload(httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(
        url=_API_URL,
        json=_openai_response({"confidence": 0.85}),
    )
    provider = OpenAIProvider(_KEY, timeout_seconds=5.0, max_attempts=1)
...
async def test_raises_validation_error_on_schema_mismatch(httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(
        url=_API_URL,
        json=_openai_response({"confidence": "not-a-number"}),
    )
    provider = OpenAIProvider(_KEY, timeout_seconds=5.0, max_attempts=1)
...
async def test_retries_on_server_error(httpx_mock: HTTPXMock) -> None:
    ...
    httpx_mock.add_response(
        url=_API_URL,
        json=_openai_response({"confidence": 0.6}),
    )
    provider = OpenAIProvider(_KEY, timeout_seconds=5.0, max_attempts=2)
    result = await provider.complete(prompt="test", schema=_SCHEMA)
